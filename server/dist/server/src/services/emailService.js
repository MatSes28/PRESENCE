...
class EmailService {
    ...
    constructor() {
        ...
        if (process.env.BREVO_API_KEY) {
            if (brevo_1.TransactionalEmailsApiApiKeys &&
                brevo_1.TransactionalEmailsApiApiKeys.apiKey) {
                this.apiInstance.setApiKey(brevo_1.TransactionalEmailsApiApiKeys.apiKey, process.env.BREVO_API_KEY);
            }
            else {
                console.warn("Brevo API key configuration is invalid - email service disabled");
            }
        }
    }
    ...
    async sendBulkAttendanceNotifications(notifications) {
        ...
        for (const notification of notifications) {
            const result = await this.sendAttendanceNotification(notification.parentEmail, notification.studentName, notification.status, notification.classInfo, notification.timestamp);
            if (result) {
                success++;
            }
            else {
