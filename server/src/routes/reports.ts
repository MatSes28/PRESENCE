...
const isValidPresetParameters = (parameters: any) =>
  !!parameters &&
...
router.get("/history/export", requireAuth, requireAdmin, reportRateLimit, async (req, res) => {
  try {
    ...
    const history = await query.orderBy(desc(reportHistory.generatedAt)).limit(1000);
    const rows = history.map((item) => {
      const parameters = (item.parameters || {}) as Record<string, any>;
      return {
        ID: item.id,
        "Report Type": item.reportType,
        Source:
          parameters.source ||
          (parameters.quickReportType ? "quick" : "manual"),
        Format:
          parameters.format ||
          (item.filePath ? item.filePath.split(".").pop() : "") ||
          "",
        Status: item.status,
        "Generated At": toIsoString(item.generatedAt),
        "Generated By": item.generatedBy?.name || "",
        "Generated By Email": item.generatedBy?.email || "",
        "Record Count": item.recordCount || 0,
        "Date Start": parameters.startDate || "",
        "Date End": parameters.endDate || "",
        Subject: parameters.subjectLabel || "All Subjects",
        "Class Section": parameters.classroomLabel || "All Sections",
        Scope: parameters.scope || "",
        "Schedule Name": parameters.scheduleName || "",
        Columns: Array.isArray(parameters.columns)
          ? parameters.columns.join("; ")
          : "",
        Filename: item.filePath?.split(/[\\/]/).pop() || "",
        "Stored Path": item.filePath || "",
        "Status Note": item.errorMessage || "",
      };
    });
    const headers = [
      "ID",
    ...
    const filename = `report-history_${formatFileDate(new Date())}.csv`;

    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);
    res.send(csv);
  } catch (error) {
    console.error("Export report history error:", error);
    res.status(500).json({
      success: false,
      message: "Failed to export report history",
    });
  }
});
...
router.post("/presets", requireAuth, async (req, res) => {
  try {
    ...
    if (!isValidPresetParameters(parameters)) {
      return res.status(400).json({
        success: false,
...
router.put("/presets/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (!allowedVisibility.includes(visibility)) {
      return res.status(403).json({
        success: false,
        message: "You do not have permission to use that preset visibility",
      });
    }
    ...
    console.error("Update report preset error:", error);
    res.status(500).json({
      success: false,
...
router.post("/presets/:id/duplicate", requireAuth, async (req, res) => {
  try {
    const presetId = parseInt(req.params.id, 10);
    const userId = Number(req.session?.userId);
    const isAdmin = req.session?.userRole === "admin";
    const { name, visibility = "personal" } = req.body;
    const allowedVisibility = isAdmin
      ? ["personal", "shared", "admin"]
      : ["personal"];
    ...
    if (
      !isAdmin &&
      sourcePreset.createdBy !== userId &&
      sourcePreset.visibility !== "shared"
    ) {
      return res.status(403).json({
        success: false,
        message: "You do not have permission to duplicate this preset",
      });
    }
    ...
    console.error("Duplicate report preset error:", error);
    res.status(500).json({
      success: false,
      message: "Failed to duplicate report preset",
    });
  }
});
