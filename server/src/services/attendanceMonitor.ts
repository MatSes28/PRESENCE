...
class AttendanceMonitor {
  ...
  async processSensorTrigger(trigger: SensorTrigger) {
    try {
      ...
      if (recentScans.length === 0) {
        ...
        return { success: false, message: 'Sensor trigger without RFID validation' }
      }
  ...
  private async findActiveClassSession(currentTime: Date) {
    ...
    const sessions = await db
      ...
      .from(classSessions)
      .where(and(
        gte(classSessions.date, new Date(currentTime.getTime() - 60 * 60 * 1000)), // Within last hour
        lte(classSessions.date, new Date(currentTime.getTime() + 60 * 60 * 1000))  // Within next hour
      ))
  ...
  private async createAttendanceRecord(record: Partial<typeof attendanceRecords.$inferInsert>) {
    const [newRecord] = await db
      .insert(attendanceRecords)
      .values({
        ...record,
        isValid: record.rfidDetected && record.sensorDetected,
        discrepancyFlag: !(record.rfidDetected && record.sensorDetected)
      })
      .returning()
  ...
  private async updateAttendanceRecord(
    ...
    updateData.isValid = (existingRecord.rfidDetected || updateData.rfidDetected) &&
  ...
  private async createDiscrepancyRecord(trigger: SensorTrigger, sessionId: number) {
  ...
  async getAttendanceStats(sessionId: number) {
    ...
    const stats = {
      ...
      rfidOnly: records.filter(r => r.rfidDetected && !r.sensorDetected).length,
      sensorOnly: records.filter(r => r.sensorDetected && !r.rfidDetected).length
    }
